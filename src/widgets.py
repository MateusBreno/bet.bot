...
def tirar_notificacoes(
    browser: Firefox, fast: WebDriverWait, wait: WebDriverWait
):
    try:
        browser.switch_to.frame(
            encontra_elementos(wait, ".lp-UserNotificationsPopup_Frame ")[0]
        )
        apertar_botao(wait, "#remindLater")
        try:
            for _ in range(2):
                apertar_botao(
                    fast, ".pm-PushTargetedMessageOverlay_CloseButton "
                )
                break
        except:
            pass
...
def devolve_jogos(wait: WebDriverWait) -> list:
    return encontra_filhos(
        encontra_elementos(wait, ".him-Classification ")[0],
        ".him-DetailsTwoWay ",
    )
...
def filtra_tempo(jogo: WebElement, maximo: int) -> bool:
    try:
        hora_min = encontra_filhos(jogo, ".him-InPlayTimer ")[0].text
    except:
        return False
...
def tirar_aposta(wait: WebDriverWait):
    try:
        apertar_botao(
            wait, ".bss-BetslipStandardModule_Minimised .bss-DefaultContent "
        )
    except:
        pass
    try:
        apostas = encontra_elementos(
            wait, ".bss-NormalBetItem_DeleteContainer "
        )
    except:
        return
    ...
    try:
        ...
        apertar_botao(wait, ".bsm-BetslipStandardModule_Overlay ")
...
def selecionar_info_tabela(
    opcao: WebElement, info: str, medida: str, minOdd: float, search: str
) -> WebElement or bool:
    rowName = (
        ".srb-ParticipantLabelCentered "
        if search == "table1"
        else ".srb-ParticipantLabel_Name "
    )
    row = -1
    for index, linha in enumerate(encontra_filhos(opcao, rowName)):
    ...
    botao = encontra_filhos(
        encontra_filhos(opcao, ".gl-Market ")[column], columnName
    )[row]

    texto_btn = botao.text.split()
    if len(texto_btn) == 2:
        texto_btn = float(texto_btn[1])
    else:
        texto_btn = float(texto_btn[0])
    if texto_btn > minOdd:
        return botao
...
def selecionar_info_tabela2(
    opcao: WebElement, info: str, medida: str, minOdd: float
) -> WebElement or bool:
    column = 0
    for index, coluna in enumerate(
        encontra_filhos(opcao, ".gl-MarketColumnHeader ")
    ...
    for index, linha in enumerate(
        encontra_filhos(
            encontra_filhos(opcao, ".gl-Market ")[column],
            ".gl-ParticipantCentered",
        )
    ):
        ...
        if medida.lower() == texto:
            ...
            if float(odd) > minOdd:
                ...
                print(
                    f"Odd inferior ao requerido: {float(botao.text)} < {minOdd}"
                )
            break
...
def seleciona_info_botoes(
    opcao: WebElement, info: str, minOdd: float
) -> WebElement or bool:
    info = re.escape(info).replace("X", "\d").lower()
...
def procura_aposta(
    opcao: WebElement, info: str or list, minOdd: float, search: str
) -> bool:
    if search != "options":
        coluna, medida = info
        if search != "table2":
            ...
            botao = selecionar_info_tabela2(opcao, coluna, medida, minOdd)
    else:
...
def adicionar_valor(
    wait: WebDriverWait, fast: WebDriverWait, title: str, valor: float
) -> bool:
    atribuiu = False

    if atribuir_valor_single(wait, title, valor):
        return True
...
def atribuir_valor_multi(
    wait: WebDriverWait, title: str, valor: float
) -> bool:
    jogadas = encontra_elementos(wait, ".bss-NormalBetItem_ContentWrapper ")
    ...
    for jogada in reversed(jogadas):
        ...
        if re.search(title, jogada.text.lower().strip()):
            encontra_filhos(jogada, ".bss-StakeBox_StakeValueInput")[
                0
            ].send_keys(str(valor))
            return True
    return False
...
def atribuir_valor_single(
    wait: WebDriverWait, title: str, valor: float
) -> bool:
    try:
        title = re.escape(title).replace("X", "\d").lower().replace("°", "º")
        text_jogada = encontra_elementos(wait, ".qbs-NormalBetItem_Details")[
            0
        ].text
...
def banca(wait: WebElement) -> float:
    texto_banca = encontra_elementos(wait, ".hm-Balance ")[0].text
    print("Banca:", texto_banca)
    return float(texto_banca.strip("R$").replace(",", "."))


def nome_times(wait: WebDriverWait) -> Tuple[str]:
    return encontra_elementos(wait, ".ipe-EventHeader_Fixture")[0].text.split(
        " v "
    )


def numero_gols(jogo: WebElement) -> List[int]:
    lista_gols = encontra_filhos(jogo, ".him-StandardScores_Scores ")[
        0
    ].text.split("\n")
...
def numero_escanteios(wait: WebDriverWait) -> int:
    texto_escanteios = encontra_elementos(wait, ".sip-MarketGroup_Info ")[
        0
    ].text
...
def esperar_sumir(wait: WebDriverWait, selector: str):
    wait.until(EC.invisibility_of_element((By.CSS_SELECTOR, selector)))
