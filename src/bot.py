...
class BetBot:
    def __init__(self, config: dict, Updater: object):
        ...
        self.golsFilter = (
            self.config["filters"]["golsFilter"][1]
            if self.config["filters"]["golsFilter"][0]
            else False
        )
    ...
    def bateu_stop(self) -> bool:
        if (
            self.saldo > self.config["settings"]["stopWin"]
            or self.saldo > self.config["settings"]["stopLoss"]
        ):
            return True
        return False
    ...
    def percorrer_jogos(self):
        ...
        def replace_team_names(
            column: list or str, casa: str, fora: str
        ) -> str or list:
            ...
            if filtra_tempo(jogo, self.config["filters"]["maxTime"]):
                ...
                for aposta in self.config["search"]:
                    ...
                    if aposta:
                        ...
                        adicionar_valor(self.wait, self.fast, title, value)
